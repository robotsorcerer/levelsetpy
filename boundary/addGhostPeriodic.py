...
def addGhostPeriodic(dataIn, dim, width=None, ghostData=None):
    ...
    # fill ghost cells
    indicesIn[dim] = sizeIn[dim] - list(range(width + 1,sizeIn[dim]));
    indicesOut[dim] = list(range(1, width));
    dataOut[indicesOut[:]] = dataIn[indicesIn[:]];

    indicesIn[dim] = list(range(1, width));
    indicesOut[dim] = sizeOut[dim] - list(range(width + 1, sizeOut[dim]))
    dataOut[indicesOut[:]] = dataIn[indicesIn[:]];
