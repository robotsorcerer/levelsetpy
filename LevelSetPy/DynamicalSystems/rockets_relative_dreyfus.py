...
class RocketSystemRel():
    ...
    def hamiltonian(self, t, value, value_derivs, finite_diff_bundle):
        ...
        Hxp = -cp.cos(self.u_p)*cp.abs(self.a*p1)+cp.cos(self.u_e)*cp.abs(self.a*p1) \
              - cp.sin(self.u_p)*cp.abs(self.a*p2)-cp.sin(self.u_e)*cp.abs(self.a*p2) \
               + self.u_p * cp.abs(p3) - self.u_e * cp.abs(p3)
    ...
    def dissipation(self, t, data, derivMin, derivMax, \
        ...
        if dim==0:
            return cp.abs(self.a*cp.cos(self.u_p)) + cp.abs(self.a*cp.cos(self.u_e))
        elif dim==1:
            return cp.abs(self.a*cp.sin(self.u_p)) + cp.abs(self.a*cp.sin(self.u_e))
        elif dim==2:
