...
def shapeRectangleByCorners(grid, lower, upper):
    """
    ...
    data = np.max(grid.xs[0] - upper[0], lower[0] - grid.xs[0]);
    for i in range(1, grid.dim):
        data = np.max(data, grid.xs[i] - upper[i])
        data = np.max(data, lower[i] - grid.xs[i])
