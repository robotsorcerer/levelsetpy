...
% Ian Mitchell, 4/23/01
  ...
  if(left)					% left trajectory
    switch bcase(1)
    case 1
      y = 2 * sin(t) - sin(theta) - (beta + 2 * t - 2 * pi - theta) * cos(t);
    case 2
      y = sin(t) - beta * cos(pi + theta / 2) + sin(theta - t) - sin(theta);
    otherwise
    ...
    switch bcase(2)
    case 1
      y = 2 * sin(t) + sin(theta) - (beta + 2 * t + theta) * cos(t);
    case 2
      y = -sin(theta) - (beta - theta) * cos(t);
    otherwise
