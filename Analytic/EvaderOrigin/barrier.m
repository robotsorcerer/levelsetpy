...
%    the game of two idenical cars with the EVADER at the origin
%
% see techrep
...
% start with right barrier
sigmaE = -1;
left = 0;

% compute the portion of the negative barrier lying on a single trajectory
if(bcase(2) == 2)

  % start with type 3 trajectory, then type 2
  %   all of the type 2 trajectory is on the barrier
  [ z2, z1, z12 ] = optTraj2(3, 2, sigmaE, sigmaE, p.tau1m, p.tau2m, ...
                             [ 0; -beta; 0 ], dt);
  xmStar.single = z1;
...
if((bcase(1) == 1) | (bcase(1) == 2))
  psis = [ p.psi0p : dpsi : pi + theta / 2, pi + theta / 2 ];
  zs = zeros(length(psis), 3);
  for i = 1 : length(psis)
    psi = psis(i);
    t2 = finalTime(theta, 0, psi, left);
