...
def fig_cbs(out_dir):
    ...
    ax.add_patch(FancyArrowPatch((7.05, 3.55), (7.95, 3.55), arrowstyle="<->",
                                 ...
                                 zorder=9))
    ax.annotate("Edge (swap) conflict\n$\\langle a_1,a_3,(5,8)\\!\\to\\!(5,9),t\\rangle$",
    ...
    ax.set_title("Discrete MAPF CBS/ECBS/RHCR "
                 "Search ", fontsize=16.5, fontweight="bold", color=INK,
                 pad=14)
    fig.subplots_adjust(left=0.01, right=0.99, top=0.9, bottom=0.02)
    p = os.path.join(out_dir, "floor_cbs.jpg")
...
def fig_ccbs(out_dir):
    fig, axes = plt.subplots(1, 2, figsize=(16.5, 8), dpi=170,
                             gridspec_kw=dict(width_ratios=[1.05, 1.0], wspace=0.16))

    # ---------- left: disc agents on a 2^k neighbourhood, continuous time -----
    ax = axes[0]
    for r in range(5):
        ax.plot([-0.5, 4.5], [r - 0.5, r - 0.5], color="#c8cde0", lw=1.2, zorder=2)
    for c in range(6):
        ax.plot([c - 0.5, c - 0.5], [-0.5, 3.5], color="#c8cde0", lw=1.2, zorder=2)
    rad = 0.42
    ...
    ax.add_patch(Circle(meet, rad * 1.9, facecolor="none", edgecolor=GOLD,
                        lw=2.4, zorder=7))
    ax.annotate("Geometrical primitives overlap.",
                xy=(1.9, 1.9), xytext=(2.9, 3.05), fontsize=22.5,
                fontweight="bold", color=GOLD, ha="center",
                arrowprops=dict(arrowstyle="->", color=GOLD, lw=1.9))
    # ax.text(-0.4, -1.0, "Arbitrary durations, "
    #                     "$2^k$-neighbourhood", fontsize=18.5, color=INK,
    #         fontweight="bold")
    ax.set_xlim(-1.0, 5.0); ax.set_ylim(3.6, -1.4)
    ax.set_aspect("equal"); ax.axis("off")
    ax.set_title("Continuous-time CBS",
                 fontsize=25, fontweight="bold", color=INK, pad=10)
    ...
    ax.add_patch(Rectangle((4.0, -0.14), 2.4, 0.28, facecolor="#f3c9c9",
                           edgecolor=A2, lw=1.6, zorder=4))
    ax.text(4.4, 1.42, "Unsafe interval for $a_1$", fontsize=22.5,
            fontweight="bold", color=A2, ha="center")
    ax.text(5.2, -0.52, "Unsafe interval for $a_2$", fontsize=22.5,
            fontweight="bold", color=A2, ha="center")
    ax.text(-0.15, 1.0, "$a_1$", fontsize=22.5, fontweight="bold", color=A1,
            ha="right", va="center")
    ax.text(-0.15, 0.0, "$a_2$", fontsize=22.5, fontweight="bold", color=A2,
            ha="right", va="center")
    ax.annotate("", xy=(10.15, 0.5), xytext=(9.4, 0.5),
                arrowprops=dict(arrowstyle="->", color=INK, lw=2.0))
    ax.text(10.2, 0.5, "time", fontsize=22.5, fontweight="bold", color=INK,
            va="center")
    # ax.text(5.0, -1.15,
    #         "Constraints now time intervals, not integer timesteps.\n",
    #         fontsize=18.5, fontweight="bold", color=INK, ha="center")
    ...
    plt.tight_layout()
    p = os.path.join(out_dir, "floor_ccbs.jpg")
    fig.savefig(p, bbox_inches="tight")
    plt.close(fig)
    print("wrote", p)
