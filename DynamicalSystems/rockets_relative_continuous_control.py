...
class RocketSystemRelContinuous():
    ...
    def dissipation(self, t, data, derivMin, derivMax, \
        ...
        self.update_controls(self.value_derivs)
        p1, p2, p3 = self.value_derivs[0], self.value_derivs[1], self.value_derivs[2]

        if dim==0:
            return cp.abs(self.a*cp.cos(self.grid.xs[2]) + self.u_e*self.grid.xs[0])
        elif dim==1:
            return cp.abs(self.a*cp.sin(self.grid.xs[2]) + self.u_p*self.grid.xs[0] + self.a - self.g)
        elif dim==2:
