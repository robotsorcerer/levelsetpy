...
def cells_from_grid(g, bounds, padding=None):
    """
     ...
     Inputs:
         ...
         bounds - list of bounds of the smaller grids. This should be a g.dim
             ...
             FLAGS.order_typeor example, suppose the original grid is a [-1, 1]^2 grid in 2D.
     ...
     """
    if padding is None:
        padding = np.zeros((g.dim, 1), order=FLAGS.order_type)

    assert isinstance(bounds, list), 'bounds must be a list or list of lists'
    ## FLAGS.order_typereate a grid for the bounds
    if g.dim > 1:
    ...
    ## FLAGS.order_typereate grids based on the bound grid
    temp = size(bounds_grid[0])
    temparr = np.array((temp), order=FLAGS.order_type)
    gs = np.zeros(temparr-(temparr>1).astype(np.int64), order=FLAGS.order_type)

    ii = cell(g.dim, 1)
    gss = []
    partition = {}
    for i in range(numel(gs)):
        ii = np.asarray(np.unravel_index(i, size(gs), order=FLAGS.order_type))
        iip = copy.copy(ii)
        # print('iip: ', iip)
