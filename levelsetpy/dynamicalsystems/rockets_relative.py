...
class RocketSystemRel():
    def __init__(self, grid, u_bound=5, w_bound=5, x=None, a=32, g=64):
        ...
        if x:
            assert isinstance(x, np.ndarray) or isinstance(x, torch.Tensor), "initial state must either be a numpy or torch tensor."
            r, c = x.shape
            if r<c:
    ...
    def hamiltonian(self, t, value, value_derivs, finite_diff_bundle):
        ...
        p1_coeff = -self.a*torch.cos(self.grid.xs[2])
        p2_coeff = self.g - self.a - self.a*torch.sin(self.grid.xs[2])
        p31_coeff = torch.abs(p1*self.grid.xs[0] + p3)
        p32_coeff = torch.abs(p2*self.grid.xs[0] + p3)
    ...
    def dissipation(self, t, data, derivMin, derivMax, \
        ...
        if dim==0:
            return torch.abs(-self.a*torch.cos(self.grid.xs[2])) + torch.abs(self.u_e*self.grid.xs[0])
        elif dim==1:
            return torch.abs(self.g - self.a -self.a*torch.sin(self.grid.xs[2])) + torch.abs(self.u_p*self.grid.xs[0]) # - self.a - self.g)
        elif dim==2:
            return abs(self.u_p + self.u_e)
