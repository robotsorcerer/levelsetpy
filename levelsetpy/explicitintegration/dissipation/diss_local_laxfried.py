...
def artificialDissipationLLF(t, data, derivL, derivR, schemeData):
    """
     artificia.dissipationLLF: local Lax-Friedrichs dissipation calculation.

     [ diss, stepBound ] = ...
                     artificia.dissipationLLF(t, data, derivL, derivR, schemeData)
    ...
    for i in range(grid.dim):
        ...
        derivMin[i] = np.minimum(derivL[i], derivR[i])
        derivMax[i] = np.maximum(derivL[i], derivR[i])
