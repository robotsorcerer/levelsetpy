...
We first introduce the notations that are commonly througout this article. Reachable sets within the context of  two person games~\cite{Isaacs1965} and their accompanying  ``viscous" terminal HJI \pde~\cite{Evans1984}\ are then  introduced. This is followed by the HJ-Isaacs PDEs commonly used to characterize reachable sets. %We then conclude this section by relating HJ PDE solution to recent results in envelopes and proximal operators.

\subsection{Notations and Terminologies}
\label{sec:back::notations}
%We adopt standard vector-matrix notations throughout. 
Conventions: Bold font upper-case and lower-case Roman letters are matrices and vectors, respectively; calligraphic letters are sets. Exception: the subdifferential \textit{set} of a BUC function, $\term(\state)$, is denoted $\subdiffset$. Time variables \eg $t_0, t, \tau, T$ will always be real positive numbers. The $n$-dimensional Euclidean space is  $\reline^n$; the set of natural numbers is $\bb{N}$. The natural log of $T$ is denoted $\log T$. The dot product is  $\langle \cdot, \cdot \rangle$. The partial time derivative of $\valuefunc$ is  $\partial \valuefunc_t$ while its partial spatial derivative is denoted $\partial \valuefunc_{\state}$. For a function $f(x(t), y(t))$ with every argument parameterized by $t$,  we simply write $f(t; x, y)$. We represent the real-valued Hilbert space with $\hilbert$; denote $2^{\hilbert}$ by its power set, \ie the family of all subsets of $\hilbert$. Associated with $\hilbert$ is the maximally monotone set-valued operator, $\bm{Q}: \hilbert \rightarrow 2^{\hilbert}$. The zeroes of $\bm{Q}$ are denoted $\text{zer } \bm{Q} = \{\state \in \hilbert \mid 0 \in \bm{Q} \state\}$. The set of global minimizers of $\term(\state)$ is denoted $\texttt{Argmin } \term(\state)$. It is written as $\argmin \, \term(x)$ when the set is a singleton. The set of fixed point of operator $S: \hilbert\rightarrow \hilbert$ is $\text{ Fix } S = \{\state \in H \mid S \state = \state\}$.

The unique solution $\state \in \ren$ of the dynamical system $\dot{\state}(\tau) = f(t; \state,  \control, \bm{w})$ is influenced by a pursuing player $\pursuer$ (with control $\bm{w} \in \mc{W}  \subseteq \reline^p$) and its evading pair $\evader$ (with control $\control \in \mc{U} \subseteq \bb{R}^m$). System trajectories $\traj_{\state, \tau}^{\control, \bm{w}}$ emanate from $\dot{\state}(\tau)$ by minimizing a scalar-valued objective 
%
\begin{align}
...
\end{align} 
%
within a two-person differential game for a fixed $T \ge \tau > t$. The controllers belong in compact sets which are  measurable functions
%
\begin{align}
...
%
that satisfies $\bm{g}: \bb{R}^n \rightarrow \bb{R}$. The differential game's \textit{lower value}~\cite{Souganidis}
%
\begin{subequations}
	\begin{align}
...
\end{subequations}
%
 is the viscosity solution to \eqref{eq:ivp}~\cite[Lemma 2.1]{Souganidis} 	with Hamiltonian, 
%
\begin{align}
...
while the  $\pursuer$ simultaneously seeks to prevent this from happening. Since we are in a two-player best-response scenario, the invariant set $\targetset_0$ obtained at $T$
\begin{align}
...
is the closure of the open set~\cite{Mitchell2020}
%
\begin{align}
...
\end{align}
%
Read: The set of states from which the strategies of $\pursuer$ and for all controls of $\evader$ imply that we \textit{reach and remain in the target set} in the interval $[-T, 0]$.  Following Lemma 2 of \cite{Mitchell2005}, the states in the reachable set admit the following properties w.r.t the value function $\lowervalue$:
%
\begin{subequations}
	\begin{align}
...
%
\begin{inparaenum}[(a)]
	\item the dynamics $f(\cdot)$ is usually nonlinear with coupled states in most problems of interest; 
	\item although  $\mc{L}([\tau, 0], \mathcal{L}_0)$ is mostly smooth, it possesses discontinuous ``shocks" at solution crossover points (see~\cite{Mitchell2020}); this makes global gradient-based optimization impossible for resolving $\valuefunc$;
...
\subsection{Cole-Hopf Transformation and the Heat Equation} % of the HJI Reachability Problem}

For the BUC function $\valueterm(t; \state): \ren \rightarrow \reline$ from \eqref{eq:ivp}, consider the  transformation $\colehopf^\delta(\state) = \exp\left({-\valuefunc^\delta}/{\delta}\right)$ originally attributed to Cole and Hopf~\cite[\S 4.4.1]{EvansPDEBook}. Applying the Cole-Hopf transformation to  \eqref{eq:ivp-viscous}, we can write
%
%\begin{subequations}
\begin{align}
\partial \colehopf^\delta_t(\state, t) &- \dfrac{\delta}{2} \colehopf^\delta(\state, t) = 0  \text{ in } \ren \times (0, T] \nonumber \\
\colehopf^\delta(\state, t) &= \exp\left({-\bm{g}}/{\delta}\right)   \text{ on } \ren \times \{t=0\}\footnote{Where we have dropped the arguments for ease of readability.}.
\tag{Heat-Kernel}
\label{eq:Cole-Hopf}
\end{align}
%
The fundamental solution, $\fundamentalsol_{\delta t}$ to \eqref{eq:Cole-Hopf} at time $t$ is~\cite{EvansPDEBook}
%
\begin{align}
...
The heat equation \eqref{eq:Cole-Hopf} admits the following explicit formulae (as a convolution, an integral expression, and a Gaussian expectation; see derivation in \cite[Appendix B]{HJMAD}),
%
\begin{subequations}
	\begin{align}
	%\tag{Cole-Hopf}
	\colehopf^\delta(t; \state) &= \left(\fundamentalsol_{\delta t} \star \exp(-\bm{g}/\delta)\right)(\state), 
	\label{eq:heat_sol_conv}\\
	&\equiv \int_{\ren} \left(\fundamentalsol_{\delta t}(\state-\bm{y})  \exp(-\bm{g}(\bm{y})/\delta)\right)d\bm{y}, \\
	%
	&=\bb{E}_{y\sim\mc{N}(\state, \delta t)}\left[\exp(-\delta^{-1} \, \term(\bm{y}))\right].
	\label{eq:heat_sol_expect}
	\end{align}
	\label{eq:heat_sol}
\end{subequations}
%
The equation \eqref{eq:Cole-Hopf} transforms solutions to the heat equation~\cite{EvansPDEBook, HJProx} to those of the viscous HJ PDE. It has enjoyed use in studying the energy landscape of neural networks~\cite{BaldassiLocalEntropy, Chaudhari2018} and viscous HJ loss functions~\cite{HJMAD}. 
%
Inspecting \eqref{eq:Cole-Hopf}, we may write
%
\begin{subequations}
	\begin{align}
	\localentropy &:= \valuefunc^\delta(t; \state) \\
	&= -\delta \log \left(\colehopf^\delta(t; \state)\right)(\state) \text{ in } \ren \times (0, T],
	\end{align}  
...
%
\begin{align}
	\partial\valuefunc_{\state}^\delta(t; \state) = - \delta \cdot \dfrac{	\partial \colehopf_{\state}^\delta(t; \state)}{\colehopf^\delta(t; \state)}.
	\label{eq:spatial_cole_hopf}
\end{align}
%
Chaudhari et al.~\cite{Chaudhari2018} called $\localentropy$ the local entropy and postulated a stochastic differential equation (SDE) scheme that leverages $\localentropy$ in SGD contexts in resolving the viscous  HJ PDE. Recent publications treat HJ PDE optimization~\cite{HJProx} in light of this local entropy function %with proximal operators and Moreau envelopes provide
by combining Moreau envelopes~\cite{Moreau} and proximal operator~\cite{BauschkeCombettes} algorithmics to establish zero-order optimization schemes for the viscous HJ loss function (with guaranteed convergence to global optima).
