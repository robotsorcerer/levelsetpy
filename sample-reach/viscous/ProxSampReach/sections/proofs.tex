...
In this section, we construct the machinery we use in building our results.

\subsection{Unique Solution to the viscous HJ Equation~\eqref{eq:ivp-viscous}}
Let us quickly construct the solution to the viscous intial-value-HJ problem.
...
\begin{proof}[Proof of Lemma \ref{lemma:hj_solution}]
	...
	\end{align}
%
Let $\colehopf^\delta(t; \state) = \openset(\valuefunc^\delta(t; \state))$, where $\openset: \ren \rightarrow \reline$ is a smooth function. Our goal is to choose $\openset$ so that $\colehopf^\delta$ solves a linear partial differential equation. We want Differentiating, we find that $\colehopf^\delta_t = \openset^\prime(\valuefunc^\delta) \valuefunc^\delta_t$ and $\Delta \colehopf^\delta = \openset(\valuefunc^\delta) \Delta \valuefunc^\delta + \openset^{\prime\prime}(\valuefunc^\delta) \hamfunc(t; \state \partial_{\state}) \valuefunc^\delta)$. Thus, \eqref{eq:ivp-viscous} implies 
%
\begin{subequations}
\begin{align}
